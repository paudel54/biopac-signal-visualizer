...
# --- Custom axis with smart tick formatting ---
...
class TimeSeriesViewer(QMainWindow):
    ...
    def load_data(self, path: Path):
        ...
        # Cast numerics (keep 'label' as string)
        for c in df.columns:
            if c in ("timestamp_ms", "label"):
                continue
            df[c] = pd.to_numeric(df[c], errors="coerce")

        # Ensure timestamp is plain np.int64 everywhere (not pandas Int64Dtype)
        ...
        label_rows["timestamp_ms"] = label_rows["timestamp_ms"].astype(np.int64)  # ensure np.int64
    ...
    def downsample_for_view(self, df):
        """
        Resample onto a time grid at target_rate (in Hz) using nearest match.
        Forward-fill slow sensors (RH/Ta/Tbody) so 1 Hz channels plot continuously.
        Also snap labels onto the grid.
        """
        if len(df) < 3:
            return df

        # No downsampling: just sorted copy (already np.int64 timestamps)
        if self.no_ds_cb.isChecked():
            out = df.sort_values("timestamp_ms").copy()
            return out

        # --- Build a uniform time grid in ms for the chosen view rate ---
        t0 = int(df["timestamp_ms"].iloc[0])
        ...
        # --- Snap data to the grid using nearest-time merge ---
        src = df.sort_values("timestamp_ms").copy()
        src["timestamp_ms"] = src["timestamp_ms"].astype(np.int64)  # ensure np.int64
        tol = step_ms
        out = pd.merge_asof(grid, src, on="timestamp_ms", direction="nearest", tolerance=tol)

        # --- Forward/backward fill ONLY the slow sensors so they don't disappear between samples ---
        slow = ["relative_humidity", "ambient_temp", "body_temp"]
        for c in slow:
            if c in out.columns:
                out.loc[out[c].isin([-99, -99.0]), c] = np.nan  # don't propagate sentinel
                out[c] = out[c].ffill().bfill()

        # --- Put labels onto the grid too (nearest-time) ---
        ...
        # --- Recompute relative_time_s for the resampled grid ---
        out["relative_time_s"] = (out["timestamp_ms"] - out["timestamp_ms"].iloc[0]) / 1000.0

        if step_ms != self.last_step_logged:
            self.log.info(f"Resample grid step = {step_ms} ms (view≈{self.target_rate} Hz)")
    ...
    def robust_yrange(self, s):
        ...
        # ignore sentinel -99 values that can wreck scaling for temp/RH
